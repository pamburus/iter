...
// Zip takes to optional values Value[T] and Value[U] and returns optional Value with a tuple of T and U.
// The returned optional Value contains value inside only if both Value[T] and Value[U] contain values inside.
func Zip[T, U any](v Value[T], u Value[U]) Value[tuple.T2[T, U]] {
	if vv, ok := v.Unwrap(); ok {
		if uu, ok := u.Unwrap(); ok {
			return Some(tuple.New2(vv, uu))
		}
	}
...
// Value returns the inner value of type T or zero initialized value.
...
// OrDefault returns the inner value T if present, otherwise it returns provided default value.
func (o Value[T]) OrDefault(value T) T {
	if o.IsSome() {
		return o.value
	}
